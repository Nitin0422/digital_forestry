...
class LandInformationForm(forms.ModelForm):
    class Meta:
        ...
        fields = ('province', 'local_level', 'ward', 'kitta_number',
                  'area', 'gps_latitude', 'gps_longitude', 'is_greater_than_10Ha')
        ...
        if 'local_level' in self.data:
            try:
                ...
                self.fields['ward'].queryset = Ward.objects.filter(
                    local_level_id=local_level_id).order_by('name')
            except:
                pass
        elif self.instance.pk:
            self.fields['ward'].queryset = Ward.objects.filter(
                local_level_id=self.instance.local_level_id).order_by('name')

        if 'province' in self.data:
            try:
                province_id = int(self.data.get('province'))
                self.fields['local_level'].queryset = LocalLevel.objects.filter(
                    province_id=province_id).order_by('name')
            except:
                pass
        elif self.instance.pk:
            self.fields['local_level'].queryset = LocalLevel.objects.filter(
                province_id=self.instance.province_id).order_by('name')


class SeedTreeForestInformationForm(forms.ModelForm):
    class Meta:
        model = SeedTreeForestInformation
        fields = ('tree_species', 'land', 'tree_type', 'tree_code',
                  'plantation_year', 'girth_cm', 'average_height')
...
class UrbanParkTreesForestInformationForm(forms.ModelForm):
    class Meta:
